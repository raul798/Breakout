...
namespace engine
{
	namespace utilities
	{
		text_manager::text_manager(int fontSize) {

			font = TTF_OpenFont("game/assets/font.ttf", fontSize);
		}

		bool text_manager::init_font() {
		...
		unsigned int text_manager::power_two_floor(unsigned int val) {
		...
		void text_manager::render_text(std::string message, SDL_Color color, float x, float y, int size)
		{
			glLoadIdentity();
